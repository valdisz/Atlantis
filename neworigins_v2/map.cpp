...
MapBuilder* ARegionList::CreateConstrainedSurfaceLevel(int level, int xSize, int ySize, char const *name, int contients,
	...
	// for New Origins V3: no gates in Volcano zone
	for (auto &kv : builder->zones) {
		auto zone = kv.second;
		if (zone->type == ZoneType::VOLCANO) {
			for (auto &rkv : zone->regions) {
				rkv.second->region->gate = -2;
			}
		}
	}
